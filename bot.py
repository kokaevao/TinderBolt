...
async def date_button(update, context):
    ...
    await send_text(update, context, "Отличный выбор! Пригласите девушку (парня) за 5 сообщений!")
...
async def message_button(update, context):
    ...
    prompt = load_prompt(query)
    user_chat_history = "\n\n".join(dialog.list)
    my_message = await send_text(update, context, "ChatGPT🧠 думает на вариантами ответа...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["age"] = text
        await send_text(update, context, "Оцените её внешность: 1-10 балолов?")
    elif dialog.count == 3:
        ...
        my_message = await send_text(update, context,
                                     "ChatGPT🧠 занимается генерацией первого сообщения. Подождите пару секунд...")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_message.edit_text(answer)
...
app = ApplicationBuilder().token("7404398058:AAHOP7poy5O5koa3Diypay_lKH95wZ5kVKg").build()
